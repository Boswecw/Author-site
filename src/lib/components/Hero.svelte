...
<section class="text-white pt-10 pb-20 lg:pt-16 lg:pb-28 relative overflow-hidden {gradientClass}">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
    <div class="text-center mb-10">
      <GenreIcon {genre} size="lg" aria-hidden="true" />
    </div>
    ...
    <div class="grid lg:grid-cols-2 gap-10 items-center">
      <div class="text-center lg:text-left">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
          <a href={ctaLink} class="px-8 py-4 rounded-lg font-semibold text-white shadow-lg transform hover:scale-105 transition-all duration-200 {ctaStyle}">
            {ctaText}
          </a>
          <a href="/about" class="px-8 py-4 rounded-lg font-semibold bg-white/10 text-white hover:bg-white/20 border border-white/30 transform hover:scale-105 transition-all duration-200">
            Learn My Story
          </a>
        </div>
      </div>
      ...
      <div class="flex justify-center lg:justify-end">
        <img
          src={coverSrc ?? createImageFallback('Featured')}
