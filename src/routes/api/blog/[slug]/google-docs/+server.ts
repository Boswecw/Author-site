// src/routes/api/blog/google-docs/+server.ts
// Blog API endpoint for Google Docs integration (separate from newsletter API)
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    // Save to posts collection (separate from newsletters)
    ...
    const response = {
      ...
      source: 'google-docs',
      publishedAt: savedPost.publishedAt?.toString(),
      lastSyncedAt: savedPost.lastSyncedAt?.toString()
    };
...
// GET endpoint to check if Google Doc has been synced as a blog post
export const GET: RequestHandler = async ({ url, request }) => {
  try {
    ...
    // Check if this Google Doc has been synced before as a blog post
    ...
    if (syncStatus.exists && syncStatus.post) {
      return json({
        exists: true,
        post: {
          id: syncStatus.post._id?.toString(),
          title: syncStatus.post.title,
          slug: syncStatus.post.slug,
          status: syncStatus.post.status,
          publishDate: syncStatus.post.publishDate?.toString(),
          lastSynced: syncStatus.lastSynced?.toString()
        },
